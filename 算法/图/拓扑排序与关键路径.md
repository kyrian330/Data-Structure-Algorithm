...
​		我们把路径上各个活动所持续的时间之和称为路径长度，**从源点到终点具有最大长度的路径叫关键路径**，在关键路径上的活动叫关键活动。上图的 AOE 网，显然 开始 -> 发动机完成 -> 组装完成就是关键路径，长度是5.5。
...
​		求事件的最早发生的时间 etv 的过程，就是从头至尾找拓扑排序的过程，因此，求关键路径之前，需要先调用一次拓扑算法来 计算 etv 和拓扑序列列表。为此，在程序开始处声明几个全局变量
...
void CriticalPath(GraphAdjList G)
{
    ...
    {
        gettop=stack2[top2--];
        for(e = G->adjList[gettop].firstarc; e; e = e->next)        /* 求各顶点事件的最迟发生时间ltv值 */
        {
            k = e=p->adjvex;
